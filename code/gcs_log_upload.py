...
class Log2Logstash2(PipelineBase):
    ...
    def run(self, params, start_time, end_time, modules, shards):
        output_writer_spec = "scheduler.logstash.gsc_log_upload.LogstashWriter"
        yield mapreduce_pipeline.MapperPipeline(
            "log2stash",
            "scheduler.logstash.gsc_log_upload.log2stash",
...
class LogstashWriter(OutputWriter):
    ...
    def from_json(cls, state):
        state = state or {}
        return cls(state.get("app_id"), state.get("gsc_bucket"), state.get("service_name"), level=state.get("level"))
    ...
    def write(self, data):
        ...
        for app_log in app_logs:
            ...
            app_log_data = dict({"log_time": time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(app_log.time)),
...
class LogUploadHandler(webapp2.RequestHandler):
    ...
    def get(self):
        ...
        params = {
            ...
            "gsc_bucket": self.get_gcs_bucket()}
